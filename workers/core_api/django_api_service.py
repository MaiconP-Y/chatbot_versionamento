...
class DjangoApiService:
    ...
    def get_google_config() -> Optional[Dict]:
        """Busca a configuração ativa do Google Auth via API."""
        url = f"{DJANGO_BAAS_URL}config/google/get/"
        try:
            response = requests.get(url, headers=AUTH_HEADERS, timeout=5)
            if response.status_code == 200:
                return response.json()
            logger.warning(f"⚠️ Google Config não encontrada ou erro: {response.status_code}")
            return None
        except requests.exceptions.RequestException as e:
            logger.error(f"❌ Erro HTTP ao buscar Google Config: {e}")
            return None

    @staticmethod
    def update_google_token(token_data: Dict) -> bool:
        """Atualiza o token data no banco de dados via API."""
        url = f"{DJANGO_BAAS_URL}config/google/update/"
        try:
            response = requests.post(url, json={'token_data': token_data}, headers=AUTH_HEADERS, timeout=5)
            if response.status_code == 200:
                return True
            logger.error(f"❌ Falha ao atualizar token: {response.text}")
            return False
        except requests.exceptions.RequestException as e:
            logger.error(f"❌ Erro HTTP ao atualizar token: {e}")
            return False